		...
		if (cfgService.getConfig().isEmpty()) {
			cfgService.updateConfig(
					new ConfigurationEntity(
							ConstantsUtils.EMAIL_TEXT_EINKAUF_UEBERSICHT,
							ConstantsUtils.THRESHOLD,
							8.0,
							ConstantsUtils.EMAIL_TEXT_EINKAUFSMANAGEMENT,
							ConstantsUtils.EMAIL_TEXT_LAGERMEISTER));
		}

		if (einheitService.all().isEmpty()
				&& kategorieService.all().isEmpty()
				&& fbService.all().isEmpty()
				&& produktService.all().isEmpty()
				&& brotService.all().isEmpty()
